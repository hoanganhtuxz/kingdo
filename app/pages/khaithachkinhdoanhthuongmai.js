...
export default function KhaithacKDTM() {
  ...
  const get = async () => {
    try {
      const res = await API.getCommercialBusiness();
      if (res.status === 200 || res.status === 201) {
        setData(res.data.data);
      }
    } catch (error) {
      console.log("this err aybc", error);
    }
  };
                ...
                <div className="row d-flex align-items-center conver-flex-mb">
                  <div className="col-md-5">
                    ...
                    >
                      ...
                      <div className="ml-5">
                        <a
                          href={`chitiet?pk=${item?.id}&role=3`}
                          className="text-white py-1 px-4 bg-red-800s text-center font-custom-mobile rounded"
                        >
                          XEM THÊM
                        </a>
                      </div>
